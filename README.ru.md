...
**Рантайм — собранный JS под обычным `node`.** `pnpm --filter @repobuddy/shared build` собирает shared в CJS в `packages/shared/dist`; `pnpm --filter @repobuddy/api build` (nest + swc) собирает `apps/api/dist`, и контейнеры запускают `node dist/main.js` / `node dist/main.worker.js`. Единственная тонкость монорепо здесь — кросс-пакетный алиас `#shared/*` — решена тем, что в swc-сборке `#shared/*` указывает на собранный `packages/shared/dist` ([`apps/api/.swcrc`](apps/api/.swcrc)), а dev по-прежнему использует `tsx` и читает TS-исходники напрямую. Миграции всё так же идут через `tsx` (`pnpm db:migrate`) — это разовый админский шаг, не горячий путь.
...
**Экономика.** Аннотация идёт на дешёвой extraction-модели, а бюджет `LLM_BUDGET_USD_PER_INDEX` (по умолчанию 2.0) реально enforced внутри фазы аннотации: перед каждой сущностью проверяется накопленный расход, и при исчерпании оставшиеся сущности остаются без описаний — индекс при этом полностью рабочий, просто `get_summary` по ним вернёт пусто, а в UI появляется плашка. Учёт ведётся в микроцентах (`usd_micro_cents`, 1 цент = 10 000): одна аннотация стоит около 0,04 цента, и любая целочисленная единица в центах либо округлила бы её в ноль, либо — при округлении вверх — завысила бы расход примерно в 50 раз. Ранняя версия этой правки так и делала, из-за чего бюджет $2 незаметно обрывал аннотацию примерно на сотой сущности.
...
**Инженерный долг, который видно из репозитория:** нет CD (CI только типизирует и гоняет тесты, деплой ручной); `apps/api` не линтится — скрипты `lint` там заглушки, ESLint настроен только для `apps/web`; тестов во фронтенде нет вообще.
