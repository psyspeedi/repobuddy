...
- **`ADMIN_LOGINS` starts empty.** Until you set it, nobody has admin access — and admins are the only accounts that bypass quotas and the daily spend cap.
...
The important caveat is that the ledger is deliberately pessimistic. The annotation step has no `usage` data to work from, so it estimates tokens from character counts and applies `Math.ceil` to each term — a floor of 2 cents per entity against a real cost closer to 0.035 cents. In practice the default budget stops annotation around the 100th entity, and a medium repository's *recorded* spend of roughly $2.50–3.30 corresponds to about $0.10–0.20 of actual OpenAI billing. Read `LLM_BUDGET_USD_PER_INDEX` as a conservative circuit breaker, not as a dollar figure on an invoice.
