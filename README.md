...
- **The API and worker run compiled JS under plain `node`.** `pnpm --filter @repobuddy/shared build` emits the shared package as CJS to `packages/shared/dist`; `pnpm --filter @repobuddy/api build` (nest + swc) emits `apps/api/dist`, and the containers run `node dist/main.js` / `node dist/main.worker.js`. The one wrinkle a monorepo hits here — the cross-package `#shared/*` alias — is resolved by pointing the swc build's `#shared/*` at the built `packages/shared/dist` (`apps/api/.swcrc`), while dev keeps `tsx` reading the TypeScript source directly. Migrations still run via `tsx` (`pnpm db:migrate`), since they're a one-off admin step, not the hot path.
