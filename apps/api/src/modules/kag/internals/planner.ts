...
- hybrid_search({ query, limit? }) → Chunk[]   (semantic + full-text search across ALL chunks — code, docs, commit messages. The go-to for fuzzy "where is X handled" queries with no concrete identifier)
...
- find_resolution({ issueNumber }) → { status, confidence, mergedByCommits, linkedPullRequests, duplicateCandidates }   (CALL FIRST when the user asks about a specific issue — plans about "issue #N" must START with this step. Detects if the issue is already solved or being solved: scans indexed commits for "fixes #N", live-searches GitHub PRs (any state, incl. DRAFT) that reference #N, and cosine-similar closed issues. Returns a status bucket: merged | open_pr | draft_pr | stale_pr | duplicate_closed | related | none. If status != 'none', the answer should be framed around the existing resolution ("already fixed in <sha>, pull latest" / "draft PR #X — great contributor entry point") instead of investigating the bug from scratch)
...
- Use \`hybrid_search\` when there is no concrete identifier — for genuinely fuzzy semantic queries ("where is discount logic").
...
- For a SPECIFIC issue ("I want to work on issue #42", "issue #191", "помоги с #42") — the plan MUST START with \`find_resolution\` with \`issueNumber: 42\` (is there already a fix / PR / duplicate?), then \`list_issues\` with \`issueNumber: 42\` (no labels) for the issue text + relatedEntities/relatedChunks. For issue-resolution questions ALWAYS expand further — pick 2-3 top relatedEntities and run \`walkthrough\` on the entity-shaped ones (functions, classes) AND \`get_callers\` to surface who depends on each, then \`retrieve_code_chunks\` on every entity gathered (initial + walkthrough + callers). End with \`answer\` whose context includes BOTH the resolution envelope and the issue envelope — the answer step then has the resolution status + the issue text + its linked code, enough to say whether the issue is already fixed and, if not, recommend a concrete starting point.
...
const FEW_SHOTS = [
  ...
  },
  {
    ...
    plan: {
      reasoning: 'No exact symbol — fall back to hybrid search over all chunks.',
      steps: [
        { id: 's1', op: 'hybrid_search', params: { query: 'discount calculation', limit: 10 } },
        {
          id: 's2',
          op: 'answer',
          params: { question: 'Where is discount logic?', context: ['$s1'] },
        },
      ],
    },
  },
