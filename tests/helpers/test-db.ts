 ...
 * 2. Default: localhost:5532/repobuddy_test
 *
 ...
 */
const DEFAULT_TEST_DB = 'postgres://repobuddy:repobuddy@localhost:5532/repobuddy_test'
...
function assertTestDatabase(url: string): void {
  try {
    ...
    if (!dbName.endsWith('_test') && dbName !== 'repobuddy_test') {
      throw new Error(
        `Refusing to run tests against "${dbName}" — database name must end in "_test". ` +
          `Set TEST_DATABASE_URL or use repobuddy_test.`,
      )
    }
  } catch (err) {
